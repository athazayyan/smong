...
import { Bot, LoaderCircle, Radar, TriangleAlert } from "lucide-react";
    ...
    <section className="rounded-[1.4rem] border border-white/18 bg-white/90 p-4 text-ink-900 shadow-[0_18px_50px_rgba(25,10,62,0.18)] backdrop-blur">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h2 className="mt-3 font-heading text-xl font-black text-ink-900">Deteksi Otomatis</h2>
        ...
        <button
          ...
          className={`inline-flex min-h-11 shrink-0 items-center justify-center rounded-full px-4 py-2 font-heading text-xs font-black transition ${
            enabled ? "bg-purple-900 text-white shadow-[0_4px_0_#20104f]" : "bg-lavender-100 text-purple-800"
...
function getStatusCopy(status: ArDetectorStatus, enabled: boolean, disabled: boolean, errorMessage: string | undefined) {
  ...
  if (status === "loading") return "Smong sedang menyiapkan model YOLO medium di browser.";
