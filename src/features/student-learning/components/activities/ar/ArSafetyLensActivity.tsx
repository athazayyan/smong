...
import { requestSafetyLensCamera, stopCameraStream } from "@/features/student-learning/lib/ar/camera";
import { buildArWarnings, createManualDetection } from "@/features/student-learning/lib/ar/markerRules";
import { YoloObjectDetector } from "@/features/student-learning/lib/ar/yoloDetector";
...
export function ArSafetyLensActivity({ activity, onComplete, backHref }: ArSafetyLensActivityProps) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const streamRef = useRef<MediaStream | null>(null);
  const markerCounterRef = useRef(0);
  const detectorRef = useRef<YoloObjectDetector | null>(null);
  ...
  const [selectedRiskClass, setSelectedRiskClass] = useState<RiskObjectClass>(activity.riskOptions[0]?.className ?? "glass-zone");
  ...
  const isSimulationMode = lensState === "simulation" || lensState === "unsupported" || lensState === "permission-denied" || lensState === "camera-error";
  const isMarkingActive = lensState === "camera-ready" || lensState === "simulation";
  const isAutoDetectionDisabled = lensState !== "camera-ready";

  const getYoloDetector = useCallback(async () => {
    if (detectorRef.current === null) {
      detectorRef.current = new YoloObjectDetector();
  ...
  }, [getYoloDetector, isAutoDetectionEnabled, lensState]);
  ...
  async function handleStartCamera() {
    ...
    const result = await requestSafetyLensCamera();
  ...
  function handleRemoveWarning(warningId: string) {
    ...
    setManualDetections((current) => current.filter((detection) => detection.id !== detectionId));
    setModelDetections((current) => current.filter((detection) => detection.id !== detectionId));
  }
      ...
      <section className="mx-auto max-w-7xl px-4 pb-4 pt-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
          <motion.div
            ...
            className="relative h-[min(720px,calc(100svh-148px))] min-h-[520px] overflow-hidden rounded-[2rem] border border-purple-700/10 bg-purple-950 shadow-[0_28px_90px_rgba(25,10,62,0.22)] md:min-h-[620px] md:rounded-[2.5rem]"
          >
            <CameraPreview videoRef={videoRef} lensState={lensState} isSimulationMode={isSimulationMode} />
            ...
            <ObjectDetectionLayer detections={detections} isVisible={isDetectionVisible && isMarkingActive} />
            ...
            <ArUnsupportedFallback lensState={lensState} onStartSimulation={handleStartSimulation} />
          </motion.div>
